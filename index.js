...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:AkTr\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=558896668696:+55 558896668696\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '.','!','#'
blocked = []
limitawal = '999999999'
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Tchau @${num.split('@')[0]} Volte Logo Amigo 👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
					...
					levelnol: '*error* 0 °-°',
					error: {
				stick: '*Fudeo, Falhou, Faz Assim, Tenta o ".f", C Não Funcionar Reporte Ao AkTr!*',
				Iv: '𝗠𝗮𝗮𝗳 𝗹𝗶𝗻𝗸 𝘁𝗶𝗱𝗮𝗸 𝘃𝗮𝗹𝗶𝗱☹️'
				},
				only: {
					group: '[❗] Este comando só pode ser usado em grupos amigo! ❌',
					premium: '[❗] ESTE PEDIDO É SO PARA *AkTr*',
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD AKTR*',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '[❗] AkTr, E Você?? Este é um recurso especial para o AkTr Hehe ❌',
					ownerB: '[❗] AkTr, E Você?? Este é um recurso especial para o AkTr Hehe ❌',
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de AkTr. Registre-se para fazer amizade com o bot AkTr por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar AkTr17\n\n──「 AKTR BOT 」──`,
					admin: '[❗] ALA O MEMBRO COMUM QUERENDO USAR COMANDOS DE ADEMIR KKKK ❌',
					Badmin: ' [❗] Este comando só pode ser usado quando o bot se torna adm Poha KKKK, Bot Quer Ademir ❌',
				}
			}
			...
			const adminbotnumber = ["558896668696@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["558896668696@s.whatsapp.net"]//ubah nomor lo
			const premium = ["558896668696@s.whatsapp.net","558296156440@s.whatsapp.net"] 
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
		...
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("Nesse grupo, não gostamos de racismos, que isso sirva de exemplo , E Racista? Vai Se Fuder Menor KKK🚶")
		}, 4000)
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("2 segundos")
		...
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("5 segundo KKKKKKKK tchau Marmita de Negão🤙")
		}, 0)
	}
		...
		setTimeout( () => {
	        ...
	        if (messagesC.includes("botfdp")){
		if (!isGroup) return
		if (!isAntiRacismo) return
		if (isGroupAdmins) return reply('PORRA ADEMIRA')
		client.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`tchau racista ${sender.split("@")[0]} voce sera expulso deste grupo em 5 segundos`)
		...
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("5 segundo KKKKKKKK tchau otário, Mim Chama de Fdp Agora KKKKKKKKKKKKKKK😔🤙")
		}, 0)
	}
		...
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("5 segundo KKKKKK Tchau Marmita de Negão🤙")
		}, 0)
	}
	
	        if (messagesC.includes("https://m.kwai.app.com/s/")){
		if (!isGroup) return
		if (!isAntiLink) return
		if (isGroupAdmins) return reply('ademira fdp, usando link de kwai, so não vou te banir pq e admin man rlx 🙂')
		client.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`link detectado ${sender.split("@")[0]} voce sera expulso deste grupo em menos de 5 segundos pro usar poha de link de kwai KKKK`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 5000)
		setTimeout( () => {
		...
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("5 segundo KKKKKKKK tchau otário, Cringe dms esses que usa kauai")
		}, 0)
	}
	
	        if (messagesC.includes("kwai")){
		if (!isGroup) return
		if (!isAntiLink) return
		if (isGroupAdmins) return reply('ademira fdp, usando kwai, so não vou te banir pq e admin man rlx 🙂')
		client.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`link detectado ${sender.split("@")[0]} voce sera expulso deste grupo em menos de 5 segundos pro usar poha de link de kwai KKKK`)
		...
		setTimeout( () => {
	        ...
	        if (messagesC.includes("https://xvideos.com/")){
		if (!isGroup) return
		if (!isAntiLink) return
		if (isGroupAdmins) return reply('vc é admin, então n irei te dar ban por usar links, rlx 🙂')
		client.updatePresence(from, Presence.composing)
		...
		setTimeout( () => {
	        ...
	        if (messagesC.includes("priquito")){
		if (!isGroup) return
		if (!isAntiPalavrao) return
		if (isGroupAdmins) return reply('vc é admin, ent não vou te banir por falar palavrão')
		client.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`palavrao detectado ${sender.split("@")[0]} voce sera expulso deste grupo em 5 segundos`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 5000)
		setTimeout( () => {
		...
		if (!isGroup) return
		if (!isAntiPalavrao) return
		if (isGroupAdmins) return reply('vc é admin, ent não vou te banir por falar coisas inapropiadas')
		client.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`palavrao detectado ${sender.split("@")[0]} voce sera expulso deste grupo em 5 segundos`)
	...
	if (messagesC.includes("botfdp")){
		...
		if (messagesC.includes("bot")){
			...
			if (messagesC.includes("AkTr")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/AkTr.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
			...
			switch(command) {
				...
				case 'menu1':
					...
					if (args.length < 1) return reply('Cade A Prr do Txt?')
					reply(mess.wait)
				...
				case 'ssweb':
					...
					if (args.length < 1) return reply('Cadê o url *link* tio')
					tekss = body.slice(7)
					reply(mess.wait)
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'modoanime':
					...
					if ((args[0]) === 'on') {
						...
						reply(`\`\`\`✓Ativado com sucesso o modo anime no grupo, seus otakus que eu amo\`\`\` *${groupMetadata.subject}*`)
					} else if ((args[0]) === 'off') {
						anime.splice(from, 1)
						fs.writeFileSync('./database/json/anime.json', JSON.stringify(anime))
					...
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('Aszideia')
					if ((args[0]) === 'on') {
						if (isAntiRacismo) return reply('O modo AntiRacismo já estava Ativo Ademira, Se Preucupa N Man!')
						antiracismo.push(from)
						fs.writeFileSync('./database/json/antiracismo.json', JSON.stringify(antiracismo))
						reply(`\`\`\`✓Ativado com sucesso o modo antiracismo no grupo, Agora Fala Algo Que Envolva Racismo Com Alguém Ai Membros Comuns Fdp\`\`\` *${groupMetadata.subject}*`)
					} else if ((args[0]) === 'off') {
...
//@darkYT					
    ...
    break
				...
				case 'semoji':
					if (args.length < 1) return reply('onde está o emojis garai????')
					ranp = getRandom('.png')
					rano = getRandom('.webp')
				...
				case 'textblue':
					...
					break
					case 'vipmenu':
					if (!isPremium) return reply('Mano, Só O AkTr Pode Usar Este Comando Mano!' ,text, { quoted: mek })
				...
				case 'premiumlist':
					...
					break
                  case 'qrcode':
        			if (!isPremium) return reply('Mano, So O AkTr Usa Esse Comando!' ,text, { quoted: mek })
               ...
               case 'pornhub':
			   reply(mess.wait)
              	    if (args.length < 1) return reply('vsf porra, se quer porno, vai no chrome fdp')
                    teks = body.slice(9)
                    anu = await fetchJson(`https://api.arugaz/media/pornhub/search?query=${teks}`, {method: 'get'})
                    teks = `===============\n`
			   ...
			   reply(mess.wait)
			     ...
			     case 'xvideos':
              	    if (args.length < 1) return reply('vsf porra, se quer porno, vai no chrome fdp'')
                    anu = await fetchJson(`https://api.arugaz.mymedia/xvideo/search?query=${body.slice(9)}`, {method: 'get'})
                    teks = `===============\n`
            ...
            case 'bodoh':
				...
				case 'hunti':
                 ...
                 case 'setdesc':
				     ...
				     client.sendMessage(from, 'Descrição Com Novas Regras com sucesso', text, {quoted: mek})
				     break
			case 'google':
...
break
				...
				break 
					...
					if (Number(args[0]) === 1) {
						if (isAntiLink) return reply('o anti-link (com o sistema anti-xvideos, anti-kwai, e anti-youtube(teste o antyt), byAkTr) está ativo')
						antilink.push(from)
						fs.writeFileSync('./src/antilink.json', JSON.stringify(antilink))
						reply('o anti-link (com o sistema anti-xvideos, anti-kwai, e anti-youtube(teste o antyt), byAkTr) Foi Ativado No Cabaré ✔️')
					...
					if (args.length < 1) return reply('digite 1 para ativar ')
					if (Number(args[0]) === 1) {
						if (isAntiLink) return reply('o sistema Anti Palavrão Ja está ativo')
						antilink.push(from)
						fs.writeFileSync('./src/antilink.json', JSON.stringify(antilink))
						reply('o sistema Anti-Palavrao Foi Ativado Com Sucesso No Cabaré ✔️')
						client.sendMessage(from,`Atencao a todos os membros ativos deste grupo anti-palavrao. e se voce enviar um palavrão tipo, *priquit0* *Bucet4* *sua puta* etc... voce sera expulso daqui  grupo[imediatamente]`, text)
					} else if (Number(args[0]) === 0) {
						if (!isantilink) return reply('O modo de grupo anti-palavrao foi desabilitado ')
						var ini = anti.clientOf(from)
						antilink.splice(ini, 1)
						fs.writeFileSync('./src/antipalavrao.json', JSON.stringify(antilink))
						reply('Desativar grupo anti-palavrao com sucesso neste grupo ✔️')
					} else {
						reply('1 para ativar, 0 para desativar ')
						}, 0)
	                             }
...
// recursos anime, random @AkTr domina vida
          ...
          reply('Espere..')
				...
				if (!isGroup) return reply(mess.only.group)
					...
					break
			    case 'AkTr':
                    client.sendMessage(from, {displayname: ('AkTr') }, MessageType.contact, { quoted: mek})
                    client.sendMessage(from, 'wa.me' ,MessageType.text, { quoted: mek} )
                    break
                case 'fitnah':	
				case 'fake':          
                    if (!isGroup) return reply(mess.only.group)
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*Fodase*\n\n Zoas, O Link do Canal Sera Exposto Em Breve'})
					break
				case 'nsfwloli1':
					memein = await kagApi.memeindo()
				...
				case 'boanoite':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'bom dia, Amo Todos Vocês!'})
					break
				case 'bomdia':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://imgur.com/gallery/zFvzl2S`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'bom dia, Amo Vocês, Todos Vocês São Os Fodas 🔥😋'})
					break
				case 'termux':
					meme = await kagApi.memes()
           ...
           case 'shitlist':
               ...
               break
				...
				case 'dono':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* AkTr\n*YOUTUBE:* EM BREVE MENOR\n*WPP:* wa.me/558896668696\n*INSTA:* Em Breve\n\n\nEspero que tenham gostado do bot 🚩️'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
        ...
        case 'promover':
				...
				case 'rebaixar':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Pronto Chefe, Esse Aki: @${mentioned[0].split('@')[0]} Perdeu Seu ADM Com Super Sucesso!!`, mentioned, true)
				...
				case 'banir':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Esse Ai Foi Removido Com Sucesso Chefe :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
		        ...
		        case 'setfoto2':
                    ...
                    media = await client.downloadAndSaveMediaMessage(mek)
                    await client.updateProfilePicture (from, media)
                    reply('Alterado com sucesso o ícone do cabaré (ops, Grupo) heheheheh')
                    break
                case 'bc2':
					if (!isadminbot) return reply('Quem é Você?')
					if (args.length < 1) return reply('.......')
				...
				case 'setpp3':
                    ...
                    media = await client.downloadAndSaveMediaMessage(mek)
                    await client.updateProfilePicture (from, media)
                    reply('Alterado com sucesso o ícone do Cabaré (ops, grupo) hehehe')
                    break
                case 'bc3':
				...
				default:
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[ERROR]','red'), 'fudeo comando não registrado de', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
			console.log('Error : %s', color(e, 'red'))
